...
class NewsController extends abstract_controller
{
    public function __construct($route)
    {
        ...
        $this->Initialization($route, 'news', 0);
    }
    ...
    public function actionUpdate()
    {
        ...
        if (isset($_POST['arc'])) {
            helper::redirect($_POST['arc']);
            exit;
        }
        if (isset($_POST['restore'])) {
            helper::redirect($_POST['restore']);
            exit;
        }
    ...
    {
        $model = new category();
        $model->parent = boot::ARCHIVE;
        $model->save((int)$this->glava);
        $link = boot::PATH . $this->controller . $this->settings['language_c'];
        helper::redirect($link);
        exit;
    }
